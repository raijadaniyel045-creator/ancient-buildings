<template>
  <div class="min-h-screen bg-paper">
    ...
    <div class="max-w-6xl mx-auto px-4 py-8">
      ...
      >
        ...
        <div class="bg-white rounded-2xl shadow-ancient p-4 border border-gold-100">
          <div class="flex gap-4">
            <div class="flex-1 relative">
              <svg
                class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-light"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          >
            <div class="p-5 flex items-center gap-4">
              <div class="relative">
                ...
                >
                <span :class="['absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-white', friend.online ? 'bg-emerald-500' : 'bg-gray-300']" />
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                  <h4 class="font-bold text-lg text-ink-black truncate">
                    {{ friend.name }}
                  </h4>
                  <span
                    v-if="friend.online"
                ...
                <p class="text-sm text-ink-light mb-2">
                  {{ friend.bio }}
                </p>
        ...
        >
          ...
          <h3 class="text-xl font-bold text-ink-black mb-2">
            暂无好友
          </h3>
        ...
        >
          ...
          >
            <div class="p-5 flex items-center gap-4">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  <h4 class="font-bold text-lg text-ink-black">
      ...
      >
        ...
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-xl font-bold text-ink-black">
              为您推荐
            </h3>
            <p class="text-ink-light">
              根据您的兴趣为您推荐志同道合的朋友
            ...
            <div class="p-5 flex items-center gap-4">
              <div class="relative">
                <img
                  ...
                  class="w-16 h-16 rounded-full object-cover border-2 border-gold-200 group-hover:border-cyan-400 transition-colors"
                >
                <span :class="['absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-white', user.online ? 'bg-emerald-500' : 'bg-gray-300']" />
              </div>
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-cyan-600">
                  共同兴趣：{{ user.commonInterests?.join('、') || '暂无' }}
                </p>
              </div>
              <button
      ...
      >
        <div class="bg-white rounded-2xl shadow-ancient overflow-hidden border border-gold-100">
          <div class="px-6 py-4 border-b border-gold-100">
            <h3 class="font-bold text-lg text-ink-black">
              最近互动
            </h3>
          </div>
    ...
    >
      <div
        class="bg-white rounded-2xl max-w-lg w-full max-h-[80vh] overflow-hidden shadow-2xl"
        @click.stop
      >
        ...
        >
          <div class="space-y-4">
            <div
              ...
              :key="index"
              :class="['flex', msg.from === 'me' ? 'justify-end' : '']"
            >
              <img
                v-if="msg.from === 'other'"
                :src="chattingWith?.avatar"
              ...
              <div :class="['max-w-[70%] px-4 py-3 rounded-xl', msg.from === 'me' ? 'bg-gradient-to-r from-cyan-500 to-cyan-600 text-white rounded-br-none' : 'bg-gold-50 text-ink-dark rounded-bl-none']">
                ...
                <p :class="['text-xs mt-1', msg.from === 'me' ? 'opacity-80' : 'text-ink-light']">
                  {{ msg.createdAt || msg.time || '刚刚' }}
                </p>
              </div>
            </div>
            <div
              v-if="chatMessages.length === 0"
              class="text-center py-8 text-ink-light"
            >
              暂无消息，开始聊天吧
            </div>
          </div>
