<template>
  <div class="min-h-screen bg-[#FDFBF7] text-[#2C363F] font-sans selection:bg-[#D4AF37] selection:text-white pb-12">
    ...
    <div class="w-full bg-[#2C363F] pt-8 pb-16 relative overflow-hidden">
      ...
      <div class="absolute right-0 top-0 w-1/2 h-full bg-gradient-to-l from-[#8B2B2B]/40 to-transparent pointer-events-none"></div>
      ...
      <div class="max-w-7xl mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div class="relative w-full md:w-[500px] h-[280px] rounded-lg overflow-hidden border-4 border-[#FDFBF7]/10 shadow-2xl group cursor-pointer">
          <img src="https://neeko-copilot.bytedance.net/api/text_to_image?prompt=traditional%20chinese%20temple%20architecture%20foguangsi%20tang%20dynasty&image_size=landscape" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
          <div class="absolute inset-0 bg-gradient-to-t from-[#2C363F] via-transparent to-transparent opacity-80"></div>
          <div class="absolute bottom-4 left-4 text-[#FDFBF7]">
            <p class="text-xs tracking-widest font-serif opacity-80 mb-1">摄影 / 寻访晋风</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex-1 lg:w-2/3">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-[#E8DFCE]/80 p-2 mb-6 flex items-center justify-between sticky top-4 z-30">
            <div class="flex gap-1 overflow-x-auto no-scrollbar">
              ...
              >
                {{ tab.name }}
                <span v-if="activeTab === tab.id" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-0.5 bg-[#8B2B2B] rounded-t-full"></span>
              </button>
            </div>
            <div class="hidden sm:flex items-center gap-3 px-4 border-l border-[#E8DFCE]">
              <button class="text-[#A69B8D] hover:text-[#D4AF37] transition-colors"><UIcon name="i-lucide-arrow-down-up" class="w-4 h-4" /></button>
              <button class="text-[#A69B8D] hover:text-[#D4AF37] transition-colors"><UIcon name="i-lucide-filter" class="w-4 h-4" /></button>
            </div>
          </div>
          ...
          <div class="space-y-6">
            ...
            >
              <!-- 侧边品类标识带 -->
              <div class="absolute left-0 top-0 bottom-0 w-1" :class="getTopicColor(post.topicType)"></div>

              <div class="p-6 pl-8">
                <!-- 帖子信息：标题与标签 -->
                <div class="mb-4">
                  <div class="flex items-center gap-2 mb-2">
                    <span class="text-[10px] px-2 py-0.5 rounded border tracking-widest font-serif" :class="getTopicStyle(post.topicType)">
                      {{ post.topicName }}
                    </span>
                ...
                <!-- 帖子图片 (若有，采用画廊式排列) -->
                <div v-if="post.images && post.images.length > 0" class="mb-5 flex gap-2 overflow-hidden h-40 rounded-lg cursor-pointer">
                  <div v-for="(img, imgIdx) in post.images.slice(0, 3)" :key="imgIdx" class="flex-1 relative overflow-hidden">
                    <img :src="img" class="w-full h-full object-cover hover:scale-110 transition-transform duration-700" />
                    <!-- 超过3张显示蒙层 -->
                    <div v-if="imgIdx === 2 && post.images.length > 3" class="absolute inset-0 bg-black/40 flex items-center justify-center text-white font-serif tracking-wider">
                ...
                <div class="flex items-center justify-between pt-4 border-t border-dashed border-[#E8DFCE]">
                  ...
                  <div class="flex items-center gap-3">
                    <img :src="post.userAvatar" class="w-8 h-8 rounded-full border border-[#E8DFCE]" />
                    <div>
                      <div class="text-sm font-medium text-[#2C363F]">{{ post.userName }}</div>
                      <div class="text-[10px] text-[#A69B8D]">{{ post.createdAt }}</div>
                    </div>
                  </div>
                  ...
                  <div class="flex items-center gap-6">
                    <button class="flex items-center gap-1.5 text-[#8B7355] hover:text-[#8B2B2B] transition-colors group/btn">
                      <UIcon name="i-lucide-flame" class="w-4 h-4 group-hover/btn:scale-110 transition-transform" :class="{'text-[#8B2B2B] fill-[#8B2B2B]': post.liked}" />
                    ...
                    <button class="flex items-center gap-1.5 text-[#8B7355] hover:text-[#5B8C7A] transition-colors group/btn">
                      <UIcon name="i-lucide-message-square" class="w-4 h-4 group-hover/btn:scale-110 transition-transform" />
        ...
        <div class="w-full lg:w-1/3 space-y-6">
          ...
          <div class="bg-gradient-to-br from-[#8B2B2B] to-[#601A1A] rounded-xl p-6 text-[#FDFBF7] shadow-lg relative overflow-hidden">
            <svg class="absolute -right-4 -bottom-4 w-32 h-32 text-white/5 pointer-events-none" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L2 22h20L12 2zm0 3.8l7.2 14.2H4.8L12 5.8z"/></svg>
            <h3 class="text-lg font-serif font-bold mb-2">撰写金石之言</h3>
            <p class="text-xs text-white/70 mb-5 leading-relaxed">或是游记见闻，或是斗拱解析，皆可在此留下您的墨宝。</p>
            <button @click="showPostModal = true" class="w-full py-3 bg-[#FDFBF7] text-[#8B2B2B] rounded-lg text-sm font-bold shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-[#E8DFCE]/80 p-5">
            <h3 class="text-base font-serif font-bold text-[#2C363F] mb-4 flex items-center gap-2">
              <UIcon name="i-lucide-trending-up" class="w-5 h-5 text-[#D4AF37]" />
              热门探讨
            </h3>
            <div class="space-y-4">
              <div v-for="(topic, idx) in hotTopics" :key="idx" class="flex gap-3 group cursor-pointer">
                <div class="text-lg font-serif font-bold mt-0.5" :class="idx < 3 ? 'text-[#8B2B2B]' : 'text-[#D1C9BE]'">
                  0{{ idx + 1 }}
                </div>
                <div>
                  ...
                  <p class="text-[10px] text-[#A69B8D]">{{ topic.hotness }} 热度</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-[#E8DFCE]/80 p-5">
            <h3 class="text-base font-serif font-bold text-[#2C363F] mb-4 flex items-center gap-2">
              <UIcon name="i-lucide-users" class="w-5 h-5 text-[#5B8C7A]" />
              活跃同道
            </h3>
            <div class="grid grid-cols-4 gap-3">
              <div v-for="user in activeUsers" :key="user.id" class="flex flex-col items-center gap-1 group cursor-pointer">
    ...
    <!-- 悬浮发帖弹窗 (模拟) -->
    <div v-if="showPostModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-[#2C363F]/40 backdrop-blur-sm" @click="showPostModal = false"></div>
      <div class="bg-[#FDFBF7] w-full max-w-2xl rounded-2xl shadow-2xl relative z-10 overflow-hidden flex flex-col max-h-[90vh]">
        <div class="px-6 py-4 border-b border-[#E8DFCE] flex justify-between items-center bg-white">
          <h2 class="text-lg font-serif font-bold text-[#2C363F]">撰写新篇</h2>
          <button @click="showPostModal = false" class="text-[#A69B8D] hover:text-[#2C363F]"><UIcon name="i-lucide-x" class="w-5 h-5" /></button>
        </div>
        <div class="p-6 flex-1 overflow-y-auto">
          <input type="text" placeholder="添上一个引人入胜的标题..." class="w-full text-xl font-serif font-bold bg-transparent border-none focus:outline-none focus:ring-0 mb-4 text-[#2C363F] placeholder:text-[#D1C9BE]" />
          <textarea rows="8" placeholder="在此铺陈您的见解与图文..." class="w-full bg-transparent border-none focus:outline-none focus:ring-0 resize-none text-[#5C544E] text-base leading-relaxed placeholder:text-[#A69B8D]"></textarea>
        </div>
        <div class="px-6 py-4 border-t border-[#E8DFCE] bg-white flex justify-between items-center">
          <div class="flex gap-2">
            <button class="p-2 text-[#8B7355] hover:bg-[#F5F0E6] rounded-lg"><UIcon name="i-lucide-image" class="w-5 h-5" /></button>
            <button class="p-2 text-[#8B7355] hover:bg-[#F5F0E6] rounded-lg"><UIcon name="i-lucide-hash" class="w-5 h-5" /></button>
          </div>
          <button class="px-8 py-2 bg-[#8B2B2b] text-white rounded-lg text-sm font-bold shadow-md hover:bg-[#601A1A] transition-colors" @click="showPostModal = false">
            发布
          </button>
        </div>
      </div>
...
const posts = ref([
  {
    id: 'p1',
    topicType: 'discussion', topicName: '学术研讨',
    ...
    images: [],
    userName: '斗拱研究员', userAvatar: 'https://neeko-copilot.bytedance.net/api/text_to_image?prompt=chinese%20architect%20avatar&image_size=square',
    createdAt: '2小时前', likes: 128, comments: 45, liked: true
  },
  {
    id: 'p2',
